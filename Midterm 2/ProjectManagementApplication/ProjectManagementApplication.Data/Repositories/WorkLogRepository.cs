...
namespace ProjectManagementApplication.Data.Repositories
{
    public class WorkLogRepository : IWorkLogRepository
    {
        ...
        public async Task<bool> Delete(int workLogId, int taskId, string userId)
        {
        ...
        {
            return await _context.WorkLogs.Where(x => x.IsActive && x.TaskId == taskId).ToListAsync();
        }

        public async Task<bool> CheckIfTaskIsAssignedToUser(int taskId, string userId)
        {
            return await _context.Tasks.AnyAsync(x => x.IsActive == true && x.Id == taskId && x.AssigneeId == userId);
        }

        public async Task<WorkLog> GetUserWorkLogByDescription(string description, string userId)
        {
