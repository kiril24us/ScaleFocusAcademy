...
namespace ProjectManagementApplication.Data.Repositories
{
    public class TaskRepository : ITaskRepository
    {
        ...
        public async Task<bool> Delete(int taskId, int projectId, string userId)
        {
            Task task = await _context.Tasks.Where(x => x.IsActive && x.Id == taskId && x.Project.IsActive && (x.Project.UserId == userId || (x.Project.Teams.Any(x => x.IsActive)
        ...
        public async Task<List<Task>> GetAllTasks(int projectId, string userId)
        {
        ...
        {
            return await _context.Tasks.Where(x => x.IsActive && x.Id == taskId && x.Project.IsActive && (x.Project.UserId == userId || (x.Project.Teams.Any(x => x.IsActive)
                 && x.Project.Teams.Any(x => x.Members.Any(x => x.Id == userId && x.IsActive))))).FirstOrDefaultAsync();
        }
